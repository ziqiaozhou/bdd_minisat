/** \file     obdd.c
 *  \brief    OBDD implementation, where OBDDs mean BDDs that are ordered, but
 * need not be reduced. \author   Takahisa Toda
 */
#include <assert.h>
#include <math.h>
#include <stdint.h>
...
static uintmax_t nnodes =
    0; // the total number of nodes that have been created so far.

obdd_t *top_node = NULL;
obdd_t *bot_node = NULL;

uintmax_t obdd_nnodes(void) { return nnodes; }
...
static uintptr_t obdd_decompose_main(FILE *out, int n, obdd_t *p,
                                     uintptr_t (*func)(FILE *, int, int,
                                                       int *)) {
  uintptr_t total = 0; // total number of total solutions
  printf("decompose main\n");
 ...
 * \param   s       length of a in which valid values are contained, which may
 * be less than the actual length of a. \param   n       the number of variables
